...
create table rank (id bigint generated by default as identity, user_email varchar(255) not null, item_id bigint not null, score integer(50), comment varchar(5000), date_commented date not null, primary key (id));
create table purchase (id bigint generated by default as identity, booked_date date not null, user_email varchar(255) not null, item_id bigint not null, primary key (id));
...
alter table purchase add constraint FKlqrv1aj0pon999jbi5esfpvbd foreign key (user_email) references users;
alter table purchase add constraint FKlqrv1aj0pon999jbi5esfkght foreign key (item_id) references item;
